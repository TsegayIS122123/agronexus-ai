...
def analyze_image(image_data: bytes) -> Dict:
    ...
    try:
        ...
        texture_score = float(min(100, max(50, 80 + random.uniform(-15, 15))))
        size_score = float(min(100, max(50, 85 + random.uniform(-15, 15))))
        moisture_score = float(min(100, max(50, 90 - random.uniform(0, 20))))
        
        defect_count = random.randint(0, 5)
        defects_score = float(max(0, 100 - (defect_count * 15)))
        
        # Overall grade (all converted to float)
        overall_grade = float(
            color_score * 0.25 + 
            texture_score * 0.20 + 
            size_score * 0.15 + 
...
def save_quality_report(
    ...
    overall_grade = float(analysis["overall_grade"])
    color_score = float(analysis["color_score"])
    texture_score = float(analysis["texture_score"])
    size_score = float(analysis["size_score"])
    moisture_score = float(analysis["moisture_score"])
    defects_score = float(analysis["defects_score"])
    
    standard_check = check_export_standards(overall_grade, product_category)
